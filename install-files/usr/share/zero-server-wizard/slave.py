...
class Slave:
	...
	def start_gui(self,standalone=False):
		...
		self.replication_speed_label=builder.get_object("replication_iface_speed_label2")
		...
		self.replication_combobox=builder.get_object("replication_combobox2")
		...
		self.replication_disabled_rbutton=builder.get_object("disabled_nfs_slave_radiobutton")
		
		self.croom_spinbutton=builder.get_object("croom_spinbutton")
		self.croom_spinbutton.connect("changed",self.spin_changed)
		
		self.internal_ip_switch=builder.get_object("internal_ip_switch")
		self.internal_ip_switch.connect("notify::active",self.internal_ip_switch_toggled)
		
		
		self.nfs_frame=builder.get_object("nfs_frame1")
		self.mount_from_master_radiobutton=builder.get_object("mount_from_master_radiobutton")
		self.mount_nfs_radiobutton=builder.get_object("mount_nfs_radiobutton1")
		self.mount_nfs_radiobutton.connect("toggled",self.mount_nfs_rb_toggled)
		self.nfs_ip_entry=builder.get_object("nfs_ip_entry1")
	...
	def internal_ip_switch_toggled(self,widget,param):
		
		self.internal_ip_entry.set_sensitive(widget.get_active())
		self.internal_mask_entry.set_sensitive(widget.get_active())
		
	#def internal_ip_switch_toggled
	
	def spin_changed(self,widget):
		
		value=self.croom_spinbutton.get_chars(0,-1)
		
		if len(value)>0:
		
		
			self.croom_spinbutton.set_value(int(value))
	...
	def get_gui_values(self):
		...
		self.template["mount_from_master"]=str(self.mount_from_master_radiobutton.get_active()).lower()
	...
	def check_values(self):
			...
			if self.template["remote_password"]!=self.template["remote_password2"]:
				error_msg+="* "+ _("Remote passwords do not match.")
			...
			if error_msg!="":
				...
				if self.template["enable_data_replication"]=="true":
					if self.template["mount_from_master"]=="true":
						msg+="[ NFS MOUNT OPTION ] = MOUNT FROM MASTER"  + "\n"
					else:
